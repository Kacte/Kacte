...
history = """                Welcome to War Of Robots

Robots have inhabited different planets in a distant galaxy
for  thousands of years. However, a devastating war ravaged
many  planets  and their inhabitants. Now, only two planets
remain,  both  in  terrible condition. The leaders of these
planets  roam the galaxy in search of a place to save their
populations.

The  situation  takes a turn for the worse when the leaders
encounter  a  tiny  planet  capable  of supporting only one
population.  Orion,  the leader of the Maximians, and Kyoko
the  leader  of  the Terminators, engage in a fierce battle
to  protect  their  populations.  The  fate of both planets
rests on the outcome of this conflict.
...
Game rules:
 ...
 - The robot with zero energy loses;
 - The robot that destroys the enemy first wins.
 - Robot St: red, blue and yellow. No skills added.
 - Robot Agro: magenta, green, orange. Loses 1 defense
   and gains random attack between 1 and 5.
 - Robot Tank: gold, diamond, emerald. Loses 1 attack
   and gains random defense between 1 and 5.
...
class Robot:
    ...
    def bonus_part(self, color_code):
      for part in self.parts:
        if color_code in colors.values():
          if color_code in ['\033[1;31m', '\033[1;34m','\033[1;33m',]:
            part.attack_level += 1
            part.defense_level +=2

          elif color_code in ['\033[1;32m','\x1b[1;95m','\x1b[38;5;202m']:
            part.attack_level += random.randint(1, 5)
            part.defense_level -= 1
          else:
            part.attack_level -= 1
            part.defense_level += random.randint(1, 5)
...
def play():
    ...
    while playing:
        ...
        rount += 1
        if not current_robot.is_on() or enemy_robot.is_there_available_part() == False:
            playing = False
            if not robot_one.is_on() or robot_one.is_there_available_part():
              robot_two.color()
              print(f"Congratulations {robot_two.name}. You destroyed the enemy robot and gave a new home to the Terminators")
   ...
   ,         , ,         , ,
     ,     ,            , '
       ' - , _ _ _ ,  '⠀⠀''')
              ...
              robot_one.color()
              print(f"Congratulations {robot_one.name}. You destroyed the enemy robot and gave a new home to the Maximians")
